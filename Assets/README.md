...
生成规则：`AppIcon.ico` 的 16/24/32 px 用 `icon-win-taskbar.svg`（小尺寸更清晰），48 px 及以上用 `icon-win-desktop.svg`；`targetsize-48_altform-lightunplated` 用单色版；启动画面与宽徽标为品牌渐变底 + 居中桌面版图标。EXE 图标由 csproj 的 `<ApplicationIcon>` 嵌入。
